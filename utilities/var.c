...
TIFILE_t* ImportBackup(FILE *infile, TIFILE_t *tifile, int *error) {
	int i, tmp;
	tifile->backup->data1 = NULL;
	tifile->backup->data2 = NULL;
	tifile->backup->data3 = NULL;

	tmpread(infile, ERR_MISSING_LENGTH);
	...
	tmpread(infile, ERR_MISSING_ADDRESS);
	...
	for(i = 0; i < tifile->backup->length1 && !feof(infile); i++) {
		tmpread(infile, ERR_MISSING_DATA);
	...
	tmpread(infile, ERR_MISSING_CHECKSUM);
...
TIFILE_t* ImportVarData(FILE *infile, TIFILE_t *tifile, int varNumber, int *error) {
	switch (tifile->type) {
		case ROM_TYPE:
	...
	tmpread(infile, ERR_MISSING_SIZE);
	headersize = tmp;
	tmpread(infile, ERR_MISSING_SIZE);
	headersize += tmp << 8;
	...
	tmpread(infile, ERR_MISSING_TYPE);
	vartype = tmp;
	...
	if ((tifile->model == TI_73 && vartype == 0x13) ||
		...
		return ImportBackup(infile, tifile, error);
	}
	...
	if (tifile->model == TI_86 || tifile->model == TI_85) {
		//skip name length
		name_length = tifile->var->name_length = tmpread(infile, ERR_MISSING_DATA);
	...
	if (tifile->type == GROUP_TYPE) {
		...
		while (tifile != NULL) {
			...
			if (length2 < 0)
				break;
			tifile = ImportVarData(infile, tifile, ++varNumber, error);
		}
	}
	tmpread(infile, ERR_MISSING_CHECKSUM);
	tifile->chksum = tmp & 0xFF;
...
TIFILE_t* newimportvar(LPCTSTR filePath, BOOL only_check_header, int *error) {
	FILE *infile = NULL;
	TIFILE_t *tifile;
	...
	tifile = ImportVarData(infile, tifile, 0, error);
	fclose(infile);
	return tifile;
}
