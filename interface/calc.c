...
BOOL rom_load(LPCALC lpCalc, LPCTSTR FileName) {
	...
	TIFILE_t* tifile = newimportvar(FileName, FALSE, NULL);
	if (tifile == NULL) {
		return FALSE;
	}
	printf("lpcalc and tifile seem to be fine");
	lpCalc->speed = 100;
	printf("lpcalc->100 assigned");
	if (lpCalc->active) {
		calc_slot_free(lpCalc);
		printf("lpcalc slot freed");
	}
	lpCalc->model = tifile->model;
	printf("lpCalc->model = tifile->model;");
	if (tifile->type == SAV_TYPE) {
		...
		printf("calc_init done");
		LoadSlot(tifile->save, lpCalc);
		printf("loadslot done");
#ifdef WINVER
